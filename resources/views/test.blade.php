<?php
use Kreait\Firebase\Factory;
$defaultAuth = Kreait\Laravel\Firebase\Facades\Firebase::auth();
// Return an instance of the Auth component for a specific Firebase project
$appAuth = Kreait\Laravel\Firebase\Facades\Firebase::project('app')->auth();


$TEST_API='{
                                 "type": "service_account",
                                 "project_id": "aema-a9a4b",
                                 "private_key_id": "72f250a0231a99ba48598a179760e449d5fb7645",
                                 "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
                                 "client_email": "firebase-adminsdk-hlqvv@aema-a9a4b.iam.gserviceaccount.com",
                                 "client_id": "118335088215997640712",
                                 "auth_uri": "https://accounts.google.com/o/oauth2/auth",
                                 "token_uri": "https://oauth2.googleapis.com/token",
                                 "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
                                 "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-hlqvv%40aema-a9a4b.iam.gserviceaccount.com",
                                 "universe_domain": "googleapis.com"
                               }';
//$factory = (new Factory)->withServiceAccount($TEST_API);
//$database = $factory->createDatabase();
//$database = app('firebase.database');
//$reference = $database->getReference('RidersInformation');

//$documents = $database->collection('RidersInformation')->documents();

//$db = $firebase->getDatabase();
//
//$ref->getReference('cars')->orderByChild('brand')->equalTo('volvo')->limitToLast(10);

//$values = $reference->getValue();
////
////
//$snapshot = $reference->getSnapshot();
////
//$value = $snapshot->getValue();
//// or
////$value = $reference->getValue();
//dd($documents);




use Google\Cloud\Firestore\FirestoreClient;

$factory = (new Factory)
    ->withServiceAccount($TEST_API);
//    ->create();
$collectionName = 'RidersInformation'; // Replace with your Firestore collection name


$firestore = app('firebase.firestore');
$database = $firestore->database();
$reference = $database->getReference($collectionName);
$snapshot = $reference->getSnapshot();
$value = $snapshot->getValue();


echo $value;
//$firestore = $factory->createFirestore();
//
//$documents = $firestore->collection($collectionName)->documents();
//
//foreach ($documents as $document) {
//    echo 'Document ID: ' . $document->id() . PHP_EOL;
//    echo 'Data: ' . json_encode($document->data(), JSON_PRETTY_PRINT) . PHP_EOL;
//}




?>
